...
def get_current(
    ...
    cdef float r_s = r_s_ref * m.exp(theta * (t_ref - t)) * (1 + tau * (g_ref - g))
    cdef float r_sh = r_sh_ref * m.exp(omega * (t_ref - t)) * (1 + zeta * (g_ref - g))

    # Thermal voltage
    cdef float v_t = n * k_b * t / q  # 26mV

    # Short circuit current
    cdef float i_sc = i_sc_ref * g / g_ref * (1 - alpha * (t_ref - t))

    # Open circuit voltage
    cdef float v_t_mod = n * k_b * (t_ref + t / gamma) / q
    cdef float v_oc = v_oc_ref * (1 - beta * (t_ref - t)) + v_t_mod * m.log(g / g_ref)
