...
AdaptiveStride::AdaptiveStride(float minStride, float VMPP, float error) {
    Stride("Adaptive", minStride, VMPP, error);
}

float AdaptiveStride::getStride(float arrVoltage, float arrCurrent, float irradiance, float temperature) {
    minStride = error * error * VMPP / (2 * (1 - error));
    float stride = 0;
    if(arrVoltage < VMPP){
        stride = exp((VMPP - arrVoltage) / 3) - 1;
    }
    return stride + minStride;
}

AdaptiveStride::~AdaptiveStride() {
