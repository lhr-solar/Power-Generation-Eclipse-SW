...
class PVCurveTracerController:
    ...
    def connect_to_curve_tracer(self, com_port, baud_rate, parity):
        self.serial_instance = serial.Serial(port=com_port, baudrate=baud_rate, parity=parity)

    def hook_in_file(self, file_name):
        if self.file_hook is not None:
            self.file_hook.close()
        self.file_hook = open(f"{file_name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log", "w+")
...
if __name__ == '__main__':
    ...
    controller.connect_to_curve_tracer(com_port=port, baud_rate=115200, parity=serial.PARITY_NONE)
    while True:
