...
class Simulation:
    ...
    def add_datapoint(self, cycle, env_conditions, source, mppt):
        ...
        # to get this in a visible range, divide by multiplier
        self.irrads.append(env_conditions[0] / 250)
        self.temps.append(env_conditions[1] / 25)
        ...
        threshold = 0.05
        # 1 index insert_index
        if p_diff > threshold:
            p_diffA = (prev_p_diffA * (insert_index) + 1) / (insert_index + 1)
        else:
            p_diffA = (prev_p_diffA * (insert_index)) / (insert_index + 1)
        ...
        # % of cycles below threshold
        self.disp_pDiffA.insert(insert_index, p_diffA)
        # tracking efficiency of the total power generated
    ...
    def init_display(
        ...
        self.scatter1 = pg.ScatterPlotItem(
            size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 255, 255)
        )
        self.scatter1.addPoints(
            [{"pos": [self.iv_curve[3][0], self.iv_curve[3][1]], "data": 1}]
        )
        self.plt2.addItem(self.scatter1)
        self.scatter2 = pg.ScatterPlotItem(
            size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 255, 255)
        )
        self.scatter2.addPoints(
            [{"pos": [self.iv_curve[3][0], self.iv_curve[3][2]], "data": 1}]
        )
                    ...
                    [
                        self.plt.plot(
                            ...
                            pen=pg.mkPen(
                                (
                                    255,
                                    255 - set_temp[TEMP][0] * 255,
                                    255 - set_temp[IRRAD][0] * 255,
                                ),
                                width=2,
                            ),
                        ),
                    ...
                    [
                        self.plt2.plot(
                            ...
                            pen=pg.mkPen(
                                (
                                    255,
                                    255 - set_irrad[TEMP][0] * 255,
                                    set_irrad[IRRAD][0] * 255,
                                ),
                                width=2,
                            ),
                        ),
    ...
    def update_display(self, cycle_start=0, cycle_end=0, time_step=1):
        ...
        # widget 2
        self.iv_curve1.setData(x=self.iv_curve[0], y=self.iv_curve[1])
        self.iv_curve2.setData(x=self.iv_curve[0], y=self.iv_curve[2])
    ...
    def overlay_data(self, file=""):
        if file != "":
            ...
            scatterPoints = pg.ScatterPlotItem(
                size=2, pen=pg.mkPen(None), brush=pg.mkBrush(255, 255, 255)
            )
