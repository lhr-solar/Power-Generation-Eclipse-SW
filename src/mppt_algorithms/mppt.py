...
class MPPT:
    ...
    def calc_perturb_amt(self, v_ref, v_in, i_in, t_in):
        ...
        """
        v_mpp = 0.47282  # .621 # according to Sunniva # TODO UPDATE THIS ESTIMATE
        if self.stride_mode == "Optimal":
            # Piegari - we assume we know where the mpp should be and jump to there
            k = 0.1  # 10% estimation error
            v_min = (
                k * k / (2 * (1 - k)) * v_mpp
            )  # this works out to be .55% for a 10% estimation error
